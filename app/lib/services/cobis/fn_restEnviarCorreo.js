...
async function fn_restEnviarCorreo(dataReques) {

  const { destinatarios, mensaje, asunto } = JSON.parse(dataReques);

  const ENVIAR_CORREO_HOST = process.env.URL_HOST_ENVIAR_CORREO;
  ...
  try {
    ...
    const options = {
      'method': 'POST',
      'hostname': `${ENVIAR_CORREO_HOST}`,
      'port': ENVIAR_CORREO_PORT,
      'path': `${ENVIAR_CORREO_PATH}`,
    ...
    let promise = new Promise(function (resolve, reject) {
      ...
      console.log('##Request Correo:', JSON.stringify(parsedPostData, null, 2));
    ...
    await promise
      .finally(() => console.log(`Integratión Enviar Correo status: ${json_data.status}`))
      .then(result => {
...
module.exports = {
  "fn_restEnviarCorreo": fn_restEnviarCorreo,
};