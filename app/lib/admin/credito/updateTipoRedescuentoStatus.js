...
export const updateTipoRedescuentoStatus = async (req) => {
    ...
    let responsServer = {};
    const sqlString = `CALL updateTipoRedescuentoStatus(?, ?, ?)`;

    try {
        let rowsAffect = 0;
        for (let i of Object.values(reqData)) {
            const [rows] = await pool.query(sqlString, [i.id, i.state, usuario]);
        ...
        console.log('updateTipoRedescuentoStatus', error);
