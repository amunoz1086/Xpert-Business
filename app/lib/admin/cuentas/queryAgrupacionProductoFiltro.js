...
export const fnQueryAgrupacionProductoFiltro = async (req) => {

    const { pGrupo, pSubproducto } = JSON.parse(req);

    const sqlString = `CALL agrupacionProductoFiltro(?, ?)`;
    let responsServer = {};

    try {

        const [rows] = await pool.query(sqlString, [pGrupo, pSubproducto]);
