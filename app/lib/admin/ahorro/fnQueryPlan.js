...
export const fnQueryPlan = async (plan) => {

    const sqlString = `CALL queryPlan(?)`;
    let responsServer = {};

    try {

        const [rows] = await pool.query(sqlString, [plan]);

        if (rows[0].length === 0) {
            responsServer.STATUS = 202;
            responsServer.MESSAGE = `El plan ${plan} no esta registrado en la base de datos`;
