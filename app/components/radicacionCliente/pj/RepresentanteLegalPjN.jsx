...
export const RepresentanteLegalPjN = ({ contextRadClient, campoValidarAdministracion, dataCamposAdministacion = [] }) => {

    const { representanteLegalPj, actualizarRepresentanteLegalPj, clienteNuevoProspectoActualizar } = contextRadClient();
    ...
    const DRAFT_KEY = 'actualizarRepresentanteLegalPj';
    ...
    useEffect(() => {
        ...
        if (draft && Array.isArray(draft) && Object.keys(catalogo).length > 0) {
            ...
            actualizarRepresentanteLegalPj(draftConListas);
        }
        /* return () => {
    ...
    useEffect(() => {
        const limpio = limpiarListas(representanteLegalPj);
    ...
    //Web Worker
    useEffect(() => {
        workerRef.current = new Worker('/workers/queryRepresentante.js');
    ...
    useEffect(() => {
        if (
            ...
            const grupoConListas = grupoDetalle?.representanteLegal?.map(item => {
                const nuevo = { ...item, nuevo: false, isSearch: true, codParticipante: '2' };
                montarListPn(nuevo);
                return nuevo;
            });

            actualizarRepresentanteLegalPj(grupoConListas);
        }
    }, [grupoDetalle, catalogo]);


    useEffect(() => {
        const updated = [...representanteLegalPj];
        let isUpdated = false;
        try {
            ...
            if (isUpdated) actualizarRepresentanteLegalPj(updated);

        } catch (error) {
            console.log(error);
        };

    }, [representanteLegalPj, catalogo]);


    //Buscar Participantes Segun Cliente Principal
    useEffect(() => {
        try {
            const validActualizacion = JSON.parse(localStorage.getItem('draft_actualizarRepresentanteLegalPj'));
    ...
    const buscarParticipantesAlIngresar = () => {
        try {
            ...
            if (parsedDataParti.hasOwnProperty('representanteLegal')) {
                const rawCustomerReferences = {
                    "representanteLegal": parsedDataParti.representanteLegal
                };
    ...
    const onChangeInput = (e) => {
        try {
            ...
            const dataRepresentanteLegarPj = [...representanteLegalPj];

            dataRepresentanteLegarPj[posItem] = {
                ...dataRepresentanteLegarPj[posItem],
            ...
            dataRepresentanteLegarPj[posItem].nuevo = true;
            dataRepresentanteLegarPj[posItem].isSearch = false;

            switch (nameCampo) {
                case 'paisNacimiento': {
                    localStorage.removeItem('paisCode');
                    localStorage.setItem("paisCode", inputValue);
                    dataRepresentanteLegarPj[posItem].ltProvinciaNacimiento = catalogo.listDepartemento.sort((a, b) => a.value.localeCompare(b.value));
                    break;
                }
                case 'pais': {
                    localStorage.removeItem('paisCode');
                    localStorage.setItem("paisCode", inputValue);
                    dataRepresentanteLegarPj[posItem].ltDepartamentoPn = catalogo.listDepartemento.sort((a, b) => a.value.localeCompare(b.value));
                    break;
                }
                case 'provinciaNacimiento': {
                    ...
                    if (paisSelect === '170') {
                        dataRepresentanteLegarPj[posItem].ltCiudadesNac = catalogo.listCiduades
                            .filter(ciudades => ciudades.code.startsWith(inputValue))
                            .sort((a, b) => a.value.localeCompare(b.value));
                    } else {
                        dataRepresentanteLegarPj[posItem].ltCiudadesNac = catalogo.listCiduades.sort((a, b) => a.value.localeCompare(b.value));
                    };
                }
                case 'departamento': {
                    let paisSelect = localStorage.getItem('paisCode');
                    ...
                    if (paisSelect === '170') {
                        dataRepresentanteLegarPj[posItem].ltCiudadPn = catalogo.listCiduades
                            .filter(ciudades => ciudades.code.startsWith(inputValue))
                            .sort((a, b) => a.value.localeCompare(b.value));
                    } else {
                        dataRepresentanteLegarPj[posItem].ltCiudadPn = catalogo.listCiduades.sort((a, b) => a.value.localeCompare(b.value));
                    };
                }
            ...
            actualizarRepresentanteLegalPj(dataRepresentanteLegarPj);
        } catch (error) {
            console.log(error);
        };
    };
    ...
    const agregarNuevaRepresentanteLegal = async (e) => {
        ...
        if (representanteLegalPj.length === 0) {
            montarListPn(nuevaReferencia);
            actualizarRepresentanteLegalPj([...representanteLegalPj, nuevaReferencia]);
            return;
        }

        setShowLoading(true);
        const resValidar = await validarParticipante(representanteLegalPj)
        if (resValidar) {
            setShowLoading(false);
            montarListPn(nuevaReferencia);
            actualizarRepresentanteLegalPj([...representanteLegalPj, nuevaReferencia]);
    ...
    const fnContinuar = async () => {
        setShowLoading(true);
        const partipantes = representanteLegalPj;
        let contNuevos = 0;
        let contSearch = 0;
        try {
                ...
                for (let i of partipantes) {
                    if (i.nuevo) {
                        ...
                        if (resGuardar.status === 200) {
                            i.nuevo = false;
                            i.customerReference = resGuardar.data;
                            actualizarRepresentanteLegalPj([...representanteLegalPj]);
                            contProcesados++
                    ...
                    if (i.isSearch) {
                        ...
                        if (resCustomer.hasOwnProperty('CustomerRightSide')) {
                            ...
                            if (contProcesados === nSearch) {
                                setShowLoading(false);
                                return '/radicacionCliente/radicacionPj/contactosAutorizadosPj';
                            };
                        };
                    };
                };
    ...
    const cancelarOperacion = (index) => {
        const copia = [...representanteLegalPj]
        copia.splice(index, 1)
        actualizarRepresentanteLegalPj(copia);
    };
    ...
    const buscarParticipante = async (index) => {
        ...
        try {
            ...
            if (rawTipoIdentificacion[0].value !== '' && rawNumeroIdentificacion[0].value) {
                ...
                const nuevoItem = { ...rawParticipante.data[0], nuevo: false, isSearch: true, codParticipante: '2' };
    ...
    return (
        <form id='representanteLegalPj'>
            ...
            <div className="flex justify-between items-start mb-6">
                <h3 className="font-semibold">INFORMACIÓN REPRESENTANTE LEGAL</h3>
                <ButtomUsuarioProspetcto contextRadClient={contextRadClient} campoValidarAdministracion={campoValidarAdministracion} />
            </div>

            <div className='flex gap-2 mb-3'>
                <label htmlFor="">Cantidad de representantes legales</label>
                <input type="number" className='border w-14 text-center  border-gray-200 rounded-md' onChange={() => { }} value={representanteLegalPj?.length} />
            </div>

            {representanteLegalPj?.map((item, i) => (
                <div key={i + 'representaten'} className="w-full px-4 mb-6 rounded-md py-6 border border-gray-200  ">
                    <div className="flex justify-between items-center">
                        <div className='flex gap-6'>
                            <h1 className="font-bold py-3">Registro de Representante Legal</h1>
                        </div>
                    </div>
