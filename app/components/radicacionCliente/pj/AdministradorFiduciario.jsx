...
export const AdministradorFiduciario = ({ contextRadClient, campoValidarAdministracion, dataCamposAdministacion = [], listTipoSociedad }) => {


    const [tipoCliente, setTipoCliente] = useState('CC')
    const { clienteNuevoProspectoActualizar, administradorFiduciario, actualizarAdministradorFiduciario } = contextRadClient()
    ...
    const camposAndCatalogos = () => ({
        cantidadAdministradorFiduciario: '',
        tipoPersona: '',
        primerApellido: '',
        segundoApellido: '',
        primerNombre: '',
    ...
    const DRAFT_KEY = 'administradorFiduciario';
    ...
    useEffect(() => {
        ...
        if (draft && Array.isArray(draft) && Object.keys(catalogo).length > 0) {
            ...
            actualizarAdministradorFiduciario(draftConListas);
        }
        /* return () => {
    ...
    useEffect(() => {
        const limpio = limpiarListas(administradorFiduciario);
    ...
    //Web Worker
    useEffect(() => {
        workerRef.current = new Worker('/workers/wk_administradorFiduciario.js');
    ...
    useEffect(() => {
        if (
            ...
            const grupoConListas = grupoDetalle?.admFiduciario?.map(item => {
                const nuevo = { ...item, nuevo: false, isSearch: true, codParticipante: '30' };
                montarListPn(nuevo);
                return nuevo;
            });

            actualizarAdministradorFiduciario(grupoConListas);
        }
    }, [grupoDetalle, catalogo]);


    useEffect(() => {
        const updated = [...administradorFiduciario];
        let isUpdated = false;
        ...
        if (isUpdated) actualizarAdministradorFiduciario(updated);
    }, [administradorFiduciario, catalogo]);


    //Buscar Participantes Segun Cliente Principal
    useEffect(() => {
        try {
            const validActualizacion = JSON.parse(localStorage.getItem('draft_administradorFiduciario'));
    ...
    const buscarParticipantesAlIngresar = () => {
        try {
            ...
            if (parsedDataParti.hasOwnProperty('administradorFiduciario')) {
                const rawCustomerReferences = {
                    "administradorFiduciario": parsedDataParti.administradorFiduciario
                };
    ...
    const agregarAdministradorFiduciario = async (e) => {
        e.preventDefault();
        const nuevoAdministradorFiduciario = camposAndCatalogos();

        if (administradorFiduciario.length === 0) {
            montarListPn(nuevoAdministradorFiduciario);
            actualizarAdministradorFiduciario([...administradorFiduciario, nuevoAdministradorFiduciario]);
            return;
        }

        setShowLoading(true);
        const resValidar = await validarParticipante(administradorFiduciario)
        if (resValidar) {
            setShowLoading(false);
            montarListPn(nuevoAdministradorFiduciario);
            actualizarAdministradorFiduciario([...administradorFiduciario, nuevoAdministradorFiduciario]);
    ...
    const guardarParticipante = async (datosForm) => {
        ...
        if (datosForm[0].nuevo) {
            ...
            console.log(resGuardarParticipante);
            setShowLoading(false);
    ...
    const fnContinuar = async () => {
        setShowLoading(true);
        const partipantes = administradorFiduciario;
        let contNuevos = 0;
        let contSearch = 0;
            ...
            for (let i of partipantes) {
                if (i.nuevo) {
                    ...
                    if (resGuardar.status === 200) {
                        i.nuevo = false;
                        i.customerReference = resGuardar.data;
                        actualizarAdministradorFiduciario([...administradorFiduciario]);
                        contProcesados++
    ...
    const montarListPn = (camposPn) => {
        ...
        camposPn.ltCategoriaCompania = catalogo.listCategoriaCompania?.sort((a, b) => a.value.localeCompare(b.value)) || [];
    ...
    const onChangeAdmFiduciario = (e) => {
        ...
        const dataAdministradorFiduciario = [...administradorFiduciario];

        dataAdministradorFiduciario[posItem] = {
            ...dataAdministradorFiduciario[posItem],
            [nameCampo]: inputValue
        };

        dataAdministradorFiduciario[posItem].nuevo = true;
        dataAdministradorFiduciario[posItem].isSearch = false;

        if (nameCampo === 'tipoPersona') {
            if (value === 'CC') {
                montarListPn(dataAdministradorFiduciario[posItem]);
            } else if (value === 'NIT') {
                montarListPj(dataAdministradorFiduciario[posItem]);
            }
        };
        ...
        switch (nameCampo) {
            case 'paisNacimiento': {
                ...
                dataAdministradorFiduciario[posItem].ltProvinciaNacimiento = catalogo.listDepartemento.sort((a, b) => a.value.localeCompare(b.value));
                break;
            }
            case 'pais': {
                localStorage.removeItem('paisCode');
                localStorage.setItem("paisCode", inputValue);
                dataAdministradorFiduciario[posItem].ltDepartamentoPn = catalogo.listDepartemento.sort((a, b) => a.value.localeCompare(b.value));
                break;
            }
            case 'provinciaNacimiento': {
                ...
                if (paisSelect === '170') {
                    dataAdministradorFiduciario[posItem].ltCiudadesNac = catalogo.listCiduades
                        .filter(ciudades => ciudades.code.startsWith(inputValue))
                        .sort((a, b) => a.value.localeCompare(b.value));
                } else {
                    dataAdministradorFiduciario[posItem].ltCiudadesNac = catalogo.listCiduades.sort((a, b) => a.value.localeCompare(b.value));
                };
            }
            case 'departamento': {
                let paisSelect = localStorage.getItem('paisCode');
                ...
                if (paisSelect === '170') {
                    dataAdministradorFiduciario[posItem].ltCiudadPn = catalogo.listCiduades
                        .filter(ciudades => ciudades.code.startsWith(inputValue))
                        .sort((a, b) => a.value.localeCompare(b.value));
                } else {
                    dataAdministradorFiduciario[posItem].ltCiudadPn = catalogo.listCiduades.sort((a, b) => a.value.localeCompare(b.value));
                };
            }
        ...
        actualizarAdministradorFiduciario(dataAdministradorFiduciario);
    };
    ...
    const buscarParticipante = async (index) => {
        ...
        if (rawTipoIdentificacion[0].value !== '' && rawNumeroIdentificacion[0].value) {
            ...
            const nuevoItem = { ...rawParticipante.data[0], nuevo: false, isSearch: true, codParticipante: '30' };
            montarListPn(nuevoItem);

            const copia = [...administradorFiduciario];
            copia[index] = nuevoItem; // ← actualiza solo el índice actual
            actualizarAdministradorFiduciario(copia);
            setShowLoading(false);
    ...
    return (
        ...
        <form id='administradorFiduciario'>

            <div className="flex justify-between items-start mb-6">
                <h3 className="font-semibold">INFORMACIÓN ADMINISTRADOR FIDUCIARIO</h3>
            ...
            <div className='flex gap-2 mb-3'>
                ...
                <input name="nIntegrantes" type="number" className='border w-14 text-center  border-gray-200 rounded-md' onChange={() => { }} value={administradorFiduciario?.length} />
            </div>

            {
                administradorFiduciario?.map((item, i) => (

                    <div key={i + 'fiduciario'} className="w-full px-4 mb-6 rounded-md py-6 border border-gray-200  ">
                        <h1 className='my-4 font-bold'>Registro de Miembro de Administrador Fiduciario</h1>
                                ...
                                <div className='flex gap-x-5'>
                                    <label className='flex gap-x-4'>
                                        <input
                                            ...
                                            type="radio"
                                            onChange={onChangeAdmFiduciario}
                                            name={"tipoPersona-" + i}
                                ...
                                <div className={`grid grid-cols-2 gap-x-8 gap-y-4 `}>
                                    ...
                                    <div >
                                        <CampoLista
                                            ...
                                            onChangeInput={onChangeAdmFiduciario}
                                        ...
                                        <div className={`grid grid-cols-3 gap-x-8 gap-y-4 `}>
                                            <div>
                                                <CheckDesplazable
                                                    ...
                                                    onclickCheck={onChangeAdmFiduciario}
            ...
            <div className='flex justify-between items-center'>
                <ButtomAgregarItem
                    titulo={'Agregar'}
                    onClick={agregarAdministradorFiduciario}
                    clienteNuevoProspectoActualizar={clienteNuevoProspectoActualizar}
