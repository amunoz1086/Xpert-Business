self.onmessage = async function (e) {
    ...
    try {
        ...
        if (rawCustomerReferences.contactoAutorizado.length > 0) {
            let contRepre = 0;
            for (let i of rawCustomerReferences.contactoAutorizado) {
                ...
                if (+parsedStatus.status === 200) {
                    ...
                    validDetallesParticipantes.contactoAutorizado.push(JSON.parse(json.resultados));

                } else {
                    throw new Error(`${JSON.parse(rawReq.rawData).status}`);
                };


                if (contRepre === rawCustomerReferences.contactoAutorizado.length) {
                    break;
                };
            };
        };
